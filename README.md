...
- [内部パラメーターと外部パラメーターについて詳しく](#内部パラメーターと外部パラメーターについて詳しく)
- [OpenCVを使用したキャリブレーションの手順](#opencvを使用したキャリブレーションの手順)
- [実装](#実装)
  - [各パラメーターを算出して保存する、かんたんな例](#各パラメーターを算出して保存するかんたんな例)
  - [保存されたパラメーターから、歪みの補正を行うかんたんな例](#保存されたパラメーターから歪みの補正を行うかんたんな例)
- [まとめ](#まとめ)
...
OpenCVでは、これらの歪みを補正するため`cv2.undistort()`関数を使用することで、歪みの補正を行うことができます。
...
![](https://raw.githubusercontent.com/yKesamaru/webcam_calibration/master/assets/checker_board.jpg)
...
1. **既知の3Dワールド座標の設定**:
   キャリブレーションパターン（例：チェスボード）の各マスの交点をもとに、各コーナーの3Dワールド座標を定義します。

2. **2D画像座標の取得**:
   カメラでキャリブレーションパターンを撮影し、画像上での各コーナーの2D座標を検出します。
